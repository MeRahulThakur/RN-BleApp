...
export const BLEProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const startScan = useCallback(() => {
    console.debug("Mock scanning started...");
    setIsScanning(true);
  ...
  const stopScan = useCallback(() => {
    console.debug("Mock scanning stopped.");
    setIsScanning(false);
  }, []);

  // --- Fake connect ---
  const connectToDevice = useCallback(async (device: MockDevice) => {
    console.debug(`Mock connecting to ${device.name}...`);
    return new Promise<MockDevice>((resolve) => {
      setTimeout(() => {
        setConnectedDevice(device);
        setLastDevice(device);
        setIsDisconnected(false);
        console.debug(`Connected to ${device.name}`);
        resolve(device);
      }, 1000);
    });
  }, []);
  ...
  const disconnectDevice = useCallback(async () => {
    console.debug("Mock disconnecting device...");
    return new Promise<void>((resolve) => {
      setTimeout(() => {
        console.debug('Disconnected manually');
  ...
  const retryConnection = useCallback(async () => {
    ...
    for (let i = 0; i < maxRetries; i++) {
      console.debug(`Retry attempt ${i + 1}/${maxRetries}`);
      ...
      if (Math.random() > 0.5) {
        ...
        setIsDisconnected(false);
        console.debug("Mock auto-reconnect success");
        return lastDevice;
      }
    }

    console.debug("Auto-reconnect failed");
    setIsDisconnected(true);
    return null;
  ...
  const readCharacteristic = useCallback(
    async (serviceUUID: string, charUUID: string) => {
      console.debug(`Mock read from service ${serviceUUID}, char ${charUUID}`);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (Math.random() < 0.2) { // 20% chance every 10s
        console.debug("Mock random disconnect occurred");
        setConnectedDevice(null);
        setIsDisconnected(true);
        retryConnection();
      }
  ...
  useEffect(() => {
    return () => {
      console.debug("BLEProvider unmounted → cleaning up subscriptions");
      cleanupSubscriptions();
    };
  }, [cleanupSubscriptions]);
